...
class UserRegister(BaseModel):
    ...
    password: str
    email: Optional[str] = ""
    role: str = "customer"  # customer, saint, or admin
...
class SaintProfile(BaseModel):
    ...
    operating_areas: Optional[List[str]] = []
    poojas: Optional[List[dict]] = []
...
async def create_saint_profile(profile: SaintProfile, user: dict = Depends(get_current_user)):
    ...
    profile_dict["total_bookings"] = 0
    profile_dict["rating"] = 5.0
    ...
    if pooja:
        ...
        for saint in saints:
            for pooja_service in saint.get("poojas", []):
                if pooja.lower() in pooja_service.get("name", "").lower():
...
async def create_booking(booking: BookingCreate, user: dict = Depends(get_current_user)):
    ...
    for pooja in saint.get("poojas", []):
        if pooja.get("name", "").lower() == booking.pooja_type.lower():
            pooja_price = pooja.get("price")
            break

    # Fallback default price if matching pooja price is missing
    if pooja_price is None:
        pooja_price = 1000.0

    base_price = float(pooja_price)
...
async def get_similar_saints(booking_id: str, user: dict = Depends(get_current_user)):
    ...
    pooja_name = booking.get("pooja_name", "")
    ...
    for saint in saints:
        for pooja in saint.get("poojas", []):
            if pooja.get("name", "").lower() == pooja_name.lower():
                saint["id"] = str(saint.pop("_id"))
...
async def verify_payment(payment_data: PaymentVerify, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if booking and booking.get("saint_id"):
